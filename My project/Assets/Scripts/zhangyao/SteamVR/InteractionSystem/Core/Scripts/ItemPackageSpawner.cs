...
namespace Valve.VR.InteractionSystem
{
    ...
    public class ItemPackageSpawner : MonoBehaviour
    {
        ...
        public void DetachAllObject()
        {
            foreach (var item in FindObjectsOfType<Hand>())
            {
                //代表手柄未启用，设置Controller可见
                if (item.isPoseValid == false)
                {
                    item.renderModelPrefab.GetComponent<RenderModel>().displayControllerByDefault = true;
                }
                DetachObject(item);
            }
        }
